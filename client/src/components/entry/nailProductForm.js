...
const NailProductForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8000/entry/create-nail-product", data,{headers:{
        Authorization:`Bearer ${token}`
      }});

      console.log(response.data);
      toast.success("Nail product added successfully!");
      reset();
      setUploadedImages([]);
    } catch (error) {
      console.error(error.response?.data || error);
      toast.error("Failed to add nail product. Please try again.");
    } finally {
      ...
      <div className="max-w-md mx-auto bg-white rounded-lg shadow-xl overflow-hidden md:max-w-2xl">
        <div className="md:flex">
          <div className="p-8 w-full">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Nail Product Entry</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div>
                <label htmlFor="nailType" className="block text-sm font-medium text-gray-700">
                  Nail Type
                </label>
                <select
                  id="nailType"
                  {...register("nailType", { required: "Nail type is required" })}
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                >
                ...
                {errors.nailType && <p className="mt-2 text-sm text-red-600">{errors.nailType.message}</p>}
              </div>
              ...
              <button
                type="submit"
                className={`w-full mt-4 bg-indigo-600 text-white rounded-md py-2 transition-opacity duration-200 ${
                  isSubmitting ? "opacity-50 cursor-not-allowed" : ""
                }`}
                disabled={isSubmitting}
              >
                {isSubmitting ? "Submitting..." : "Add Nail Product"}
              </button>
            </form>
            <ToastContainer />
          </div>
...
export default NailProductForm;
